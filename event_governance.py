...
def build_field_base(field_name, suffix):
    words = split_pascal_case(field_name)
...
def generate_boolean_description(field_name, path):
    """Generate standardized 'True, if...' descriptions for boolean fields (PascalCase)."""
    obj_name = get_object_name(path)
    lower_name = field_name.lower()
    has_is = lower_name.startswith("is")
    has_enabled = lower_name.endswith("enabled")
    
    # Check if this is an "Active" field (case-insensitive)
    is_active_base = lower_name.replace("is", "").replace("enabled", "").strip() == "active"

    suggestions = []
    
    # Suggestion 1: Is prefix style or "Active" override
    if is_active_base:
        suggestions.append(f"True, if {obj_name} is active")
    else:
        # Strip 'Is' if it exists
        base = field_name[2:] if has_is else field_name
        words = split_pascal_case(base)
        word_list = [w.lower() for w in words]
        if len(word_list) > 1:
            suggestions.append(f"True, if {' '.join(word_list[:-1])} is {word_list[-1]}")
        else:
            suggestions.append(f"True, if {' '.join(word_list)} is {' '.join(word_list)}")

    # Suggestion 2: Enabled suffix style
    # Always include this suggestion for ambiguous/invalid names
    base = field_name[:-7] if has_enabled else field_name
    # Also strip Is prefix for the enabled suggestion if it was there
    if base.lower().startswith("is"):
        base = base[2:]
    words = split_pascal_case(base)
    word_list = [w.lower() for w in words]
    suggestions.append(f"True, if {' '.join(word_list)} is enabled")
    ...
    # Ambiguous or invalid: return both separated by ||
...
class RuleEngine:
    ...
    def apply_rules(self, field_name, field_body, current_path):
        ...
        # ------------------------------------------------
        # Step 1: Clean Eg / e.g. + Fix punctuation
        # ------------------------------------------------
        if old_description:
        ...
        if field_body.get("type") == "boolean":
            has_is = field_name.startswith("Is")
            has_enabled = field_name.endswith("Enabled")
            ...
            if (has_is and has_enabled) or (not has_is and not has_enabled):
                ...
                if has_is and has_enabled:
                    expected = f"Choose either Is prefix or Enabled suffix (not both)"
                else:
                    expected = f"Should start with Is or end with Enabled"
        ...
        # ------------------------------------------------
        # Step 5: Compare & update description
        # ------------------------------------------------
...
def extract_asyncapi_definition(data):
    ...
    if (
        data.get("kind") == "API"
        and data.get("spec", {}).get("type") == "asyncapi"
...
def main(file_path, mode, user_actions=None):
    ...
    asyncapi_data, wrapper_type = extract_asyncapi_definition(data)

    if not asyncapi_data:
        return []

    engine = RuleEngine(mode, user_actions)
    engine.traverse(asyncapi_data)
...
if __name__ == "__main__":
    if len(sys.argv) != 4 or sys.argv[2] != "--mode":
        print("Usage: python event_governance.py file.yaml --mode validate|fix")
    else:
